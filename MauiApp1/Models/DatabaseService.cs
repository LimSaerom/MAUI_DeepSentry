...
namespace MauiApp1.Models
{
    ...
    public class DBManager
    {
        ...
        public async Task Add_Document_with_AutoID()          //for 비동기 처리 (Task List 사용)
        {
            //비동기 처리 = 실시간 자료 업데이트를 방지하고, 앞선 작업이 완료될 때까지 기다림

            string detData = DateTime.Now.ToString("yyyy-MM-dd");
            string detTime = DateTime.Now.ToString("HH:mm:ss");
            string detAnimal;


            CollectionReference coll = db.Collection("LogRecord");
            List<Task> tasks = new List<Task>();  // 비동기 작업 리스트 추가

            for (int i = 0; i == 0; i++)
            {
                detAnimal = Random.Shared.Next(1, 4).ToString(); // 랜덤 숫자 생성 (1~4 사이)
                detAnimal = detAnimal switch
                {
                    "1" => "bear",
                    "2" => "waterdear",
                    "3" => "boar",
                    _ => "etc"            //switch에서 언더스코어(_)는 기본값 설정
                };
                ...
                tasks.Add(coll.AddAsync(data));  // 비동기 작업 리스트에 추가
            }
            await Task.WhenAll(tasks);  // 모든 작업이 끝날 때까지 기다림
        }
